...
- Approved English and Russian string matrix and rendered primary state for each locale, including application, widget, notification, accessibility, permission, and image-rendered text.
...
Configure English and Russian application localizations and follow the iPhone's system language. Use English as the development and fallback localization and Russian as the second fully shipped localization unless `AppSpec.md` explicitly approves another order. Do not add an in-app language selector.
...
Implement all approved user-facing copy through localized resources rather than Swift literals. Provide `ru.lproj` and `en.lproj` resources for every target that presents text, including the main application, widget, Notification Service Extension, and Notification Content Extension. Localize Info.plist purpose strings, UI labels and buttons, alerts, permission recovery, accessibility labels and values, widget configuration text, notification categories and generated content, MapKit search copy, and text drawn into result images. Use localized format strings for interpolated values instead of concatenating translated fragments. Keep analytics event names, persistence keys, notification category identifiers, deep links, bundle identifiers, backend fields, and other protocol values unchanged. Ensure shared code resolves user-facing strings from the current target's bundle.
...
7. Launch the application in English and Russian, verify English fallback behavior for an unsupported locale, inspect the approved primary and important permission states for clipping or unexpected wrapping, and verify every shipped target contains the appropriate localized resources. Verify source and processed Info.plist purpose strings in both languages against `AppSpec.md` verbatim. Do not create localization tests.
...
- Supported application localizations, development and fallback localization, localized resource paths for every shipped target, and English/Russian launch verification status.
...
Lead with what works. Provide clickable paths to the project, `AppSpec.md`, configuration, backend entry point, approved localized mockups, app-icon master, app-owned Privacy Manifests, application and permission localizations, and release manifest. Include exact build and backend-run commands, configurable values, verified Russian and English states, verified targets, Release and archive status, AppSpec conformance, production, policy, credential, and signing limitations. Mention physical-device limitations only when that verification was explicitly requested.
