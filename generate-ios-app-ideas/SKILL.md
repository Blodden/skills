...
6. **Minimal backend**:
   ...
   - Prefer exactly `GET /health`, `GET /config?appId=...&appVersion=...`, and one idea-specific `POST /sync`. Let `/sync` accept and return one compact snapshot and carry an installation identifier, lightweight authentication value, and APNs device token when practical.
   - Define exactly one remote Boolean flag, `cloudSyncEnabled`. Require every public configuration read to send a concrete numeric `CFBundleShortVersionString` as `appVersion`; do not define a versionless read. Store one optional `disabledFromVersion` threshold per `appId`: return `true` below it, `false` at or above it, and `true` for every version when no threshold exists. Compare one to three numeric version components rather than strings, so `1.10` is newer than `1.2`. When false, stop backend snapshot and APNs-token synchronization while keeping the permission-centered product usable locally. Cache the last successful value separately for each app version and use `true` before the first successful fetch for that version. Do not include the build number unless the user explicitly needs build-level targeting.
   - Keep flag reads public and non-secret. Do not propose a public write endpoint or an in-app flag switch; setting or clearing `disabledFromVersion` belongs to an authenticated Yandex Cloud console, CLI, or IAM-protected function invocation.
