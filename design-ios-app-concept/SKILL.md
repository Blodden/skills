...
- Start required local initialization and the single `cloudSyncEnabled` request as soon as the programmatic startup continuation appears. Default its app-controlled display interval to at least 0.7 seconds and at most 4 seconds; record both values in `AppSpec.md` and obtain approval for any change. If the request exceeds the maximum, design a transition using the last value cached for the current app version or `true` before that version's first successful fetch, with compact offline/fallback status on the main screen.
